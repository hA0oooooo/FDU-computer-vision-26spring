...
run_timed "$OBJECT_C_NAME" preprocess_foreground "remove background; lambda_2d_3d=${LAMBDA_2D_3D}" \
...
run_timed "$OBJECT_C_NAME" magic123_preprocess_rgba_depth "generate rgba.png and depth.png; lambda_2d_3d=${LAMBDA_2D_3D}" \
...
run_timed "$OBJECT_C_NAME" magic123_coarse "coarse implicit optimization; lambda_2d_3d=${LAMBDA_2D_3D}; lambda_guidance=${COARSE_LAMBDA_2D},40" \
...
run_timed "$OBJECT_C_NAME" magic123_fine_export "fine DMTet optimization and final mesh export; lambda_2d_3d=${LAMBDA_2D_3D}; lambda_guidance=${FINE_LAMBDA_2D},0.01" \
env CUDA_VISIBLE_DEVICES=$GPU python main.py -O \
  --text "$TEXT_PROMPT" \
  --sd_version 1.5 \
  --image "$RGBA_IMAGE" \
